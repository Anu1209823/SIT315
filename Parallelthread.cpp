...
void sequentialMatrixMultiplication(int** A, int** B, int** C, int size)
{
    for (int i = 0; i < size; ++i)
    {
        for (int j = 0; j < size; ++j)
...
int main()
{
    ...
    clock_t sequentialStartTime = clock();
    sequentialMatrixMultiplication(inputMatrix1, inputMatrix2, resultMatrixSequential, matrixSize);
    clock_t sequentialEndTime = clock();
    double sequentialExecutionTime = double(sequentialEndTime - sequentialStartTime) / CLOCKS_PER_SEC * 1000;

    // Print and write result of sequential multiplication...
    cout << "Sequential Result Matrix:" << endl;
    printMatrix(resultMatrixSequential, matrixSize);
    cout << endl;
    writeMatrixToFile(resultMatrixSequential, matrixSize, "sequential_result_matrix.txt");
    cout << "Sequential Execution time: " << sequentialExecutionTime << " milliseconds" << endl;
    ...
    for (int i = 0; i < matrixSize; ++i)
    {
        ...
        delete[] resultMatrixSequential[i];
        delete[] resultMatrixParallel[i];
    }
    delete[] inputMatrix1;
    delete[] inputMatrix2;
    delete[] resultMatrixSequential;
